import templogo from '../assets/idol_temp_logo.png'
import bloomlogo from '../assets/logos/bloomlogo_400x400.jpg'
import snacktimelogo from '../assets/logos/snacktimelogo.jpg'
import sunnylogo from '../assets/logos/sunnylogo.png'
import lovechordlogo from '../assets/logos/lovechord_logo.jpg'
import reveillogo from '../assets/logos/reveil_logo.jpg'
import undeadonlinelogo from '../assets/logos/undeadonline_logo.jpg'
import sourcandylogo from '../assets/logos/sourcandy_logo.jpg'
import vividlogo from '../assets/logos/vivid_logo.jpg'
import neonismlogo from '../assets/logos/neonism_logo.jpg'
import faeblelogo from '../assets/logos/faeble_logo.jpg'
import partofpiecelogo from '../assets/logos/partofpiece_logo.jpg'
import himizulogo from '../assets/logos/himizu_logo.jpg'
import projectheart_logo from '../assets/logos/projectheart_logo.png'
import aquaticmemories_logo from '../assets/logos/aquaticmemories_logo.jpg'
import hibiskiss_logo from '../assets/logos/hibiskiss_logo.png'
import lumeteor_logo from '../assets/logos/lumeteor_logo.jpg'
import starrydance_logo from '../assets/logos/starrydance_logo.jpg'
import unmei_logo from '../assets/logos/unmei_logo.jpg'
import amuse_logo from '../assets/logos/amuse_logo.jpg'
import seiryouprocess_logo from '../assets/logos/seiryouprocess_logo.png'
import moemagicmaids_logo from '../assets/logos/moemagicmaids_logo.jpg'
import constellation_logo from '../assets/logos/constellation_logo.jpg'
import defi_logo from '../assets/logos/defi_logo.jpg'
import yumehoshi_logo from '../assets/logos/yumehoshi_logo.jpg'

export const idolGroupsArray = [
    {
        id:1,
        name:"Bloom Idol Project",
        location:"Brisbane/Meanjin",
        logo: bloomlogo,
        genre: ["Jpop"], 
        description:`
            Hello Blossoms, We are Bloom Idol Project! 

            We are a Brisbane-based idol trio with our hearts set on captivating audiences and bringing energy and lots of sparkle to every stage we perform on! Inspired by J-pop and idol culture, our mission is to bring the crowd to life and leave you smiling! 

            Like flowers in a garden, we grow a little more with each performance! Whether it's your first time seeing idols or you're a long-time fan, we're here to brighten your day and bring a little joy to your heart! Join us as we bloom into your favorite idols, one petal at a time!🌸

            From Youtube 9/3/26
        `,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/bloom_idol_project"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@BloomIdolProject"
            },
            {
                type: "twitter",
                link: "https://x.com/bloomidols"
            }
        ]
    },
    {
        id:2,
        name:"SnackTime",
        location:"Brisbane/Meanjin",
        logo: snacktimelogo,
        genre: ["Jpop", "Genki"], 
        description:`Yum yum! We are Snack Time Idols (スナックタイム) a Kaigai Jpop Idol Trio from Brisbane. We perform singing and dancing with songs from anime, to jpop, to Australian based idol groups. Come along to watch a delicious performance that’s high energy and full of heart from our 3 appetising idols.

            Our goal is to grow our individual skills while performing energizing shows, perform across Australia and promote improvement, support and fun in our community

            From Website 9/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/snacktime.info"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@SnackTimeIdolAU"
            },
            {
                type: "website",
                link: "https://snack-time-idols.square.site/"
            }
        ]
    },
    {
        id:3,
        name:"Sunnysaurus",
        location:"Sydney/Eora",
        logo: sunnylogo,
        genre: ["Jpop", "Eurobeat", "EDM", "Rock"],
        description:`
                Passionate about community and inclusivity Sunny has been actively performing and creating live events within the Australian J-idol scene since 2019. Previously a member of MEWS Au they are now the founder and leader of idol group FAEBLE as well as a solo performer. 
                
                Currently exploring a Mahou Gyaru concept, Sunny performs a variety of original music as well as covers that focus on happiness, confidence, fashion and magic! Their sound most typically covers Eurobeat, EDM and Rock in a j-idol format. Sunny is self produced and enjoys creating music and a variety of content across their various social media channels
            From Website 9/3/26`,
        socials: [
            {
                type: "instagram",
                link: "http://instagram.com/sunnyysaurus"
            },
            {
                type: "tiktok",
                link: "https://www.tiktok.com/@sunnysaurus"
            },
            {
                type: "twitter",
                link: "https://x.com/sunnyysaurus"
            },
            {
                type: "spotify",
                link: "https://open.spotify.com/artist/0qTVRf71muCDW3EdtFcerY?si=OJaB-ISSQ5Wi4ncDano4rg"
            },
            {
                type: "website",
                link: "https://www.sunnysaurus.net/"
            }
        ]
    },
    {
        id:4,
        name:"Seiryou Process ☆ 星稜プロセス",
        location:"Brisbane/Meanjin",
        logo: seiryouprocess_logo,
        genre: ["Jpop"],
        description:`
                We are Seiryou Process! よろしくお願いします🙇‍♀️🙇‍♂️ A self-produced Australian kaigai idol duo reaching for the stars. Based in Brisbane and established in September 2022, we have been fortunate enough to perform on stages around the country and are ready to take the next step on our journey.
                
            From Patreon 10/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/sei.pro"
            },
            {
                type: "tiktok",
                link: "https://www.tiktok.com/@sei.pro"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@SeiryouProcess"
            },
            {
                type: "patreon",
                link: "https://www.patreon.com/cw/SeiryouProcess"
            }
        ]
    },
    {
        id:5,
        name:"A-MUSE PROJECT",
        location:"Brisbane/Meanjin",
        logo: amuse_logo,
        genre: ["Jpop"],
        description:`
                Beginning in 2014, A-MUSE formed as a humble one-time cosplay and dance project. Since then, we have morphed into an Australian idol group that performs  a wide variety of Japanese pop and idol music not only within Brisbane, but across Australia!  Driven by our passion for idols, we aim to make the scene more accessible for those who can't make it to Japan to partake in this fun and exciting culture.
                
            From Youtube 10/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/amuse_pro"
            },
            {
                type: "tiktok",
                link: "https://tiktok.com/@amuse_pro"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@AMUSE_PRO"
            },
            {
                type: "twitter",
                link: "https://twitter.com/amuse_pro"
            },
            {
                type: "website",
                link: "https://www.amuse-project.com/"
            },
            {
                type: "patreon",
                link: "https://www.patreon.com/amuseproject"
            },
            {
                type: "twitch",
                link: "https://twitch.tv/amuseproject"
            }
        ]
    },
    {
        id:6,
        name:"Cassie-Sama",
        location:"Brisbane/Meanjin",
        logo: templogo,
        genre: ["Eurobeat", "Rock", "Metal"],
        description:`
                Breaking through your screen and into your heart!
                It's Cassie Sama!
                
                Cassie-Sama is a solo kaigai idol based in Brisbane who captivates J-Pop enthusiasts with her unique shows. Since 2022, she has performed live throughout Australia and virtually around the globe. Her music focuses on idol culture, using Eurobeat, rock, and metal to create a unique sound. Drawing inspiration from past and future technology, experiences, and current events, Cassie-Sama's performances are both innovative and mesmerising!
                
            From Youtube 10/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/cassiesama_/"
            },
            {
                type: "tiktok",
                link: "https://www.tiktok.com/@cassie.sama"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@cassiesama2903"
            },
            {
                type: "twitter",
                link: "https://twitter.com/cassiesama"
            },
            {
                type: "spotify",
                link: "https://open.spotify.com/artist/1N2A2ppfmw1pK5RdzsEgx8"
            },
            {
                type: "website",
                link: "https://cassie-sama.wixsite.com/cassie-sama"
            },
            {
                type: "twitch",
                link: "https://www.twitch.tv/cassiesama"
            }
        ]
    },
    {
        id:7,
        name:"CH4i",
        location:"Brisbane/Meanjin",
        logo: templogo,
        genre: ["Jpop"],
        description:`
                💚 Wan Wan…NUMBER 1! Brisbane’s green soloist Ch4i is here! Just like chai is his drink of choice, Ch4i hopes to become your idol of choice, so let’s have a latte of fun together! 💚
                
                Ch4i is an independent kaigai idol based in Brisbane, Australia. With an experience in the local Australian idol community of 10 years and even longer in the performing arts industry, Ch4i is a seasoned professional with their own unique image and original music being put out into the world! 🐶
                
            From Youtube 10/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/ch4i_html/"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@Ch4i-Latte"
            },
            {
                type: "spotify",
                link: "https://open.spotify.com/artist/4qnMOJwZtk1k7t3Dzh7wk9"
            },
            {
                type: "website",
                link: "https://ch4i-html.square.site/"
            }
        ]
    },
    {
        id: 8,
        name: "LoveChord",
        location: "Sydney/Eora",
        logo: lovechordlogo,
        genre: [""],
        description:`A 4 member kaigai idol with a sweet, dark & energetic concept themed around the cycle of love!
        
        From Website 13/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/lovechord.info/"
            },
            {
                type: "tiktok",
                link: "https://www.tiktok.com/@lovechord.info"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@lovechord_info"
            },
            {
                type: "twitter",
                link: "https://x.com/lovechord_info"
            },
            {
                type: "website",
                link: "https://lovechord.square.site/"
            }
        ]
    },
    {
        id: 9,
        name: "ReVeil",
        location: "Sydney/Eora",
        logo: reveillogo,
        genre: [""],
        description:`An idol group based in Sydney/Eora, Australia! 
        
        Reveling and Revealing in moonlit hallways…but beware, the longer you experience the magic, the darker it gets.
        
        From Website 13/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/reveil.au/"
            },
            {
                type: "tiktok",
                link: "https://www.tiktok.com/@reveil.au"
            },
            {
                type: "twitter",
                link: "https://x.com/Reveil_au"
            },
            {
                type: "spotify",
                link: "https://open.spotify.com/playlist/0CGCAENTAj9viIKK34mqIQ?pi=VxRYsi6JQNaNg"
            },
            {
                type: "website",
                link: "https://reveil-idols.square.site/"
            }
        ]
    },
    {
        id: 10,
        name: "Undead Online",
        location: "Melbourne/Naarm",
        logo: undeadonlinelogo,
        genre: [""],
        description:`You can"t run and you can"t hide!! Formed in 2024, we are a Naarm, Australia based j-idol duo with an alternative monster concept. With our terrifyingly high energy and dedication we give performances that"ll keep you watching even if you"re frightened. From metal to spooky jpop, let"s raise enough energy to wake the dead!!!
        
        From Website 13/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/undead.online/"
            },
            {
                type: "tiktok",
                link: "https://tiktok.com/@undead.online"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@UndeadOnline"
            },
            {
                type: "website",
                link: "https://undead-online.square.site/"
            }
        ]
    },
    {
        id: 11,
        name: "Sour Candy",
        location: "Sydney/Eora",
        logo: sourcandylogo,
        genre: [""],
        description:`We are Sour Candy, a NSW, Australia based kaigai idol/idol cover group!
        The two members that make up Sour Candy are Bibi and Jinx.
        Represented by a pink shark is Bibi, and represented by a green demon llama is Jinx.
        Together their colours represent Sour Candy! 🖤
        
        From Youtube 13/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/sourcandy.ai/"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@sourcandy_ai"
            },
            {
                type: "website",
                link: "https://sourcandyai.square.site/"
            }
        ]
    },
    {
        id: 12,
        name: "ViVID",
        location: "Sydney/Eora",
        logo: vividlogo,
        genre: [""],
        description:`We are VIVID!
        
        オーストラリア・シドニーを拠点とする3人組の日本人ガールズグループ VIVID  由来は鮮やかという意味で3人の魅力ある姿をファンの方々に見せれるようにという願いが込められている。
        
        RUA ENTERTAINMENT. 所属
        

        VIVID is a  three-member Japanese girl group based in Sydney, Australia. The name comes from the meaning of vivid and expresses the hope that the three will be able to show their charming side to their fans.
        
        Produced by RUA ENTERTAINMENT.
        
        From Youtube 13/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/vivid.gram/"
            },
            {
                type: "tiktok",
                link: "https://www.tiktok.com/@vivid.tok"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@vividinfoo"
            }
        ]
    },
    {
        id: 13,
        name: "Naneko",
        location: "Sydney/Eora",
        logo: templogo,
        genre: [""],
        description:`💖Lolita fashion💖
        
        From Instagram 13/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/naneko_nya/"
            },
            {
                type: "website",
                link: "https://naneko-nya.square.site/"
            }
        ]
    },
    {
        id: 14,
        name: "NEONism",
        location: "Sydney/Eora",
        logo: neonismlogo,
        genre: ["Rock", "EDM"],
        description:`Sydney rock & EDM idol group
        
        From Instagram 13/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/_neonism/"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@_neonism"
            }
        ]
    },
    {
        id: 15,
        name: "FAEBLE",
        location: "Sydney/Eora",
        logo: faeblelogo,
        genre: [""],
        description:`FAEBLE are a high energy, singing and dancing idol group with a rock inspired sound based in Sydney, AUSTRALIA! With years of experience on stage and running idol events, FAEBLE’s passion is bringing fun and interactive stages to all.
        
        FAEBLE perform not only at their own events but also as guests acts and at conventions across Australia. Debuting in 2021, FAEBLE are entirely self produced and enjoy creating music and other exciting content across their social media channels.
        
        From Website 13/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/faeblefuture/"
            },
            {
                type: "tiktok",
                link: "https://www.tiktok.com/@faeblefuture"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/c/faeble"
            },
            {
                type: "twitter",
                link: "https://x.com/faeblefuture"
            },
            {
                type: "website",
                link: "https://www.faeblefuture.com/"
            }
        ]
    },
    {
        id: 16,
        name: "Part of Piece!",
        location: "Sydney/Eora",
        logo: partofpiecelogo,
        genre: [""],
        description:`We are a J-pop idol group based in Sydney, Australia.
        We want to bring a new flavour to the world!
        
        🍰 do you want a piece? 🍰
        
        From Youtube 13/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/patopi_info/"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@partofpiece"
            },
            {
                type: "spotify",
                link: "https://open.spotify.com/artist/239FjERVJh6gsVKAc8Vl0H"
            }
        ]
    },
    {
        id: 17,
        name: "DAV!NE",
        location: "Melbourne/Naarm",
        logo: templogo,
        genre: [""],
        description:`ダバイン || 海外アイドル
        idol ♡ cosplay ♡ slay
        ˗ˋˏ💎ˎˊ˗
        
        From Instagram 13/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/davinegravity/"
            },
            {
                type: "tiktok",
                link: "https://www.tiktok.com/@davinekarma"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@davinegravity"
            },
            {
                type: "twitter",
                link: "https://x.com/davinegravity"
            }
        ]
    },
    {
        id: 18,
        name: "HiMizu",
        location: "Sydney/Eora",
        logo: himizulogo,
        genre: [""],
        description:`HiMizu is a kaigai idol unit based in Eora/Sydney, Australia.
        There"s no me without you... We are HiMizu!
        
        From Website 13/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/himizu.au/"
            },
            {
                type: "tiktok",
                link: "https://www.tiktok.com/@himizu.au"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@HiMizu_au"
            },
            {
                type: "twitter",
                link: "https://x.com/himizu_au"
            }
        ]
    },
    {
        id: 19,
        name: "Project Heart",
        location: "Sydney/Eora",
        logo: projectheart_logo,
        genre: [""],
        description:`Project Heart is a Sydney based MenChika idol Project that made their stage debut in 2025.Tay, (Current member) the demon prince promises to win fans" hearts and bring them endless happiness. Their core message is that fans are their cherished ones, a special force held forever in their hearts. This concept is reflected in a strong visual style, skilfully blending elements of darkness and intensity with a passionate, unwavering theme of an idol"s profound and eternal commitment to their loyal Cherished Ones.
        
        From carrd 13/3/26`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/projectheart_info/"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@ProjectHeart_official"
            },
            {
                type: "website",
                link: "https://project-heart-idols.square.site/"
            }
        ]
    },
        {
        id: 20,
        name: 'Universal Makeup',
        location: 'Melbourne/Naarm',
        logo: unmei_logo,
        genre: [""],
        description:`うんめい ☆ Universal Makeup!!
            Eyes on this space
            🩷🩵💚

            Taken from Instagram on 17/03/2026`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/unmei_idols"
            }
        ]
    },
    {
        id: 21,
        name: 'Hibiskiss',
        location: 'Melbourne/Naarm',
        logo: hibiskiss_logo,
        genre: [''],
        description:``,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/hibiskiss.idols/"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@Hibiskiss-idols"
            }
        ]
    },
    {
        id: 22,
        name: 'Starry Dance',
        location: 'Melbourne/Naarm',
        logo: starrydance_logo,
        genre: [''],
        description:`Just a bunch of girls chasing that glow ✨

            We’re a dance team from Melbourne, Australia 
            — mostly K-pop, sometimes not 🇦🇺

            Taken from YouTube on 17/03/2026`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/starrydanceofficial"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@StarryDanceOfficial"
            }
        ]
    },
    {
        id: 23,
        name: 'LuMeteor',
        location: 'Melbourne/Naarm',
        logo: lumeteor_logo,
        genre: [''],
        description:`We are a Melbourne-based Kaigai IdolGroup!
            Mizu Member: Haru Harurun
            Pastel Purple: Meiry
            Mint: Kagami
            Red: Tsuki

            Taken from YouTube on 17/03/2026`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/lumeteor_aus/"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@LuMeteor_melb"
            },
            {
                type: "website",
                link: "https://lumeteor.square.site/"
            }
        ]
    },
    {
        id: 24,
        name: 'Aquatic Memories',
        location: 'Melbourne/Naarm',
        logo: aquaticmemories_logo,
        genre: [''],
        description:``,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/aquatic_memories"
            }
        ]
    },
    {
        id: 25,
        name: "Moe Magic Maids",
        location: "Brisbane/Meanjin",
        logo: moemagicmaids_logo,
        genre: [""],
        description:`Welcome Home Princesses, Masters!
        Okaerinasaimase ojōsama ōji-sama!~
        お帰りなさいお嬢様王子様!~
        We are Moe Magic Maids!!
        魔法の萌えメイド

        We are starting a kickstarter to help our business to bring Japanese maid cafe culture to Australia! 🫶

        What is a maid cafe? 🩵
        A maid cafe, founded in the hearts of Akihabara Japan 2001, is an entertainment cafe where waitresses dressed in maid costumes serving, cute themed foods, live shows and doing fun interactions with customers. A fun place for all ages to enjoy and being entertained during their duration inside the cafe!

        We plan to pop up to collaborate with stores in Brisbane QLD Australia and hoping to expand Australia wide and open Australias very first permanent maid cafe. But we need your help! any donations made will 100% be funded in this project. Let’s bring our hearts together, Moe Moe Kyun!!
        Each donation will get gifts once we launch as a thank you ♡

        Taken from ko-fi on 19/03/2026`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/moemagicmaids"
            },
            {
                type: "tiktok",
                link: "https://www.tiktok.com/@moemagicmaids"
            },
            {
                type: "ko-fi",
                link: "https://ko-fi.com/moemagicmaids"
            }
        ]
    },
        {
        id: 26,
        name: "De:Fi",
        location: "Brisbane/Meanjin",
        logo: defi_logo,
        genre: ["Jpop"],
        description:`We are De:Fi! A pink and blue brisbane-based idol duo 💙💕

        Taken from Tiktok on 21/03/2026`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/defi.official/"
            },
            {
                type: "tiktok",
                link: "https://www.tiktok.com/@de..fi"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@defi.official"
            }
        ]
    },
    {
        id: 27,
        name: "Constellation",
        location: "Brisbane/Meanjin",
        logo: constellation_logo,
        genre: ["Jpop"],
        description:`Established in 2017 as a Love Live! Idol group, Constellation are a Brisbane based kaigai idol and dance group who love to share their passion and energy on stage for all things J-idol!

        Taken from Website on 21/03/2026`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/constellation_aus/"
            },
            {
                type: "tiktok",
                link: "https://www.tiktok.com/@constellation_aus"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@constellation_aus"
            },
            {
                type: "website",
                link: "https://constella-tion.carrd.co/"
            }
        ]
    },
    {
        id: 28,
        name: "Yume☆Hoshi",
        location: "Brisbane/Meanjin",
        logo: yumehoshi_logo,
        genre: ["Jpop"],
        description:`『夢にキラめく、お星さまアイドル』
        ☆ kaigai idol group based in Brisbane☆
        Taken from Instagram on 21/03/2026`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/yumehoshi_idol/"
            },
            {
                type: "tiktok",
                link: "https://www.tiktok.com/@yumehoshi_idol"
            }
        ]
    },
        {
        id: 29,
        name: "Coda",
        location: "Sydney/Eora",
        logo: templogo,
        genre: [""],
        description:`CODA the UNDEAD is an overseas idol, risen from the grounds of Sydney, Australia.

        Since her debut in 2020, Coda has been blasting through live stages across Australia, with explosive, genre-bursting sets that blend the sounds of metal, edm and J-pop

        Taken from website 21/03/2026`,
        socials: [
            {
                type: "instagram",
                link: "https://instagram.com/undeadcoda"
            },
            {
                type: "tiktok",
                link: "https://tiktok.com/@undeadcoda"
            },
            {
                type: "twitter",
                link: "https://twitter.com/undeadcoda"
            },
            {
                type: "website",
                link: "https://www.codatheundead.net/"
            },
            {
                type: "youtube",
                link: "https://www.youtube.com/@coda_the_undead"
            },
            {
                type: "twitch",
                link: "https://twitch.tv/undeadcoda"
            }
        ]
    },
    {
        id: 30,
        name: "Wooly",
        location: "Sydney/Eora",
        logo: templogo,
        genre: [""],
        description:`A solo idol based in Australia!

        Taken from Ko-Fi 21/03/2026`,
        socials: [
            {
                type: "instagram",
                link: "https://www.instagram.com/wooly_chan"
            },
            {
                type: "twitter",
                link: "https://x.com/wooly_chan"
            }
        ]
    },

];